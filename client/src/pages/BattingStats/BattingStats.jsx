...
const BattingStats = () => {
    ...
    const load = async () => {
      try {
        const [bRes, dRes] = await Promise.all([
          fetch(`${API_BASE}/api/players/batting-analytics/${playerName}`),

          fetch(`${API_BASE}/api/players/player/${playerName}`),
        ]);
        ...
        if (dData.impactScore) {
          setDetailed((prev) =>
            prev ? { ...prev, impactScore: dData.impactScore } : prev,
          );
        }
      } catch (err) {
        console.error(err);
  ...
  const verdict =
    strikeRate > 150
      ? { label: "Elite Striker", color: "var(--ci-brand)" }
      : strikeRate > 130
        ? { label: "Strong Striker", color: "var(--ci-brand)" }
        : strikeRate > 110
          ? { label: "Solid Batter", color: "var(--ci-blue)" }
      ...
      <main className={styles.main}>
        ...
        {phases.length === 0 && dotBallPct === null && (
          <div className={styles.noDetailBanner}>
            ...
            <span>
              Full phase and breakdown analytics not available for this player.
            </span>
          </div>
        )}
      </main>
